...
const AGENT_BUILT_AT: string = (function() {
  try { return new Date(statSync(__filename).mtime).toISOString(); } catch(e) { return "unknown"; }
})();
...
// Orario reale per IP ricevuti via BanSync fleet-wide su iptv_ban (POST /api/ipset/:name/add):
// non hanno una riga in bans.log locale perche' il ban e' avvenuto su un altro VPS, quindi
// registriamo qui il momento in cui QUESTO VPS ha ricevuto/applicato l'IP.
const iptvBanTimeByIp: Record<string, string> = {};
...
app.get("/api/banned-ips", async (_req, res) => {
  try {
    ...
    // Aggiungi IP da ipset iptv_ban (anti-iptv.sh). Orario reale: prima le righe
    // "DATA: ... / IP BANNATO: <ip>" di bans.log (ban rilevato localmente da questo VPS),
    // altrimenti il momento in cui BanSync ha propagato l'IP qui (iptvBanTimeByIp), altrimenti now().
    ...
    const iptvLocalBanTimeByIp: Record<string, string> = {};
    let iptvLogMatch;
...
app.post("/api/unban", async (req, res) => {
  ...
  if (jail === "anti-iptv") {
    ...
    if (result.ok) delete iptvBanTimeByIp[ip];
    bannedIpsCache = null;
...
app.get("/api/stats", async (_req, res) => {
  try {
    ...
    // Sum currently-banned across all fail2ban jails (in parallelo, vedi nota in /api/banned-ips)
...
app.post("/api/fail2ban/filters/:name/test", async (req, res) => {
  ...
  const filterPath = path.resolve(path.join(FILTER_DIR, name + ".conf"));
  if (!filterPath.startsWith(FILTER_DIR + "/")) return res.status(400).json({ error: "Invalid filter name" });
  const { logline } = req.body;
  if (!logline || typeof logline !== "string" || logline.length > 2000) {
  ...
  try {
    fd = openSync(tmpFile, fsc.O_WRONLY | fsc.O_CREAT | fsc.O_EXCL | fsc.O_NOFOLLOW, 0o600);
    writeSync(fd, logline + "\n");
    closeSync(fd);
    fd = null;
    const result = await runCmd("fail2ban-regex " + tmpFile + " " + filterPath + " 2>&1", 15000);
    ...
    if (fd !== null) { try { closeSync(fd); } catch(_) {} }
    await runCmd("rm -f " + tmpFile);
    res.status(500).json({ error: err.message });
  }
});
...
function antiIptvScriptPath(variant: string): string {
  return variant === "sh" ? "/usr/local/sbin/anti-iptv.sh" : "/usr/local/sbin/anti-iptv.py";
}

function detectAntiIptvVariant(): string {
...
// Regex a coppie [ricerca, indice-gruppo-valore] per estrarre i parametri dal contenuto live dello script.
// Bash: "MAX_USERNAME=4" (valore diretto nella variabile).
// Python: "MAX_USERNAME = int(os.environ.get(\"MAX_USERNAME\", \"3\"))" (default hardcoded, nessun override env).
var ANTI_IPTV_FIELD_PATTERNS: Record<string, Record<string, RegExp>> = {
  sh: {
...
function parseAntiIptvFields(variant: string, content: string): { maxUsername: number | null; windowSeconds: number | null; banSeconds: number | null } {
  var patterns = ANTI_IPTV_FIELD_PATTERNS[variant];
  var out: { maxUsername: number | null; windowSeconds: number | null; banSeconds: number | null } = { maxUsername: null, windowSeconds: null, banSeconds: null };
...
app.post("/api/anti-iptv/whitelist", async (req, res) => {
  try {
    ...
    // MAI `sudo sh -c`: pgagent non ha /bin/sh nei sudoers, quindi questa
    // persistenza falliva in silenzio (stesso gap del 2026-07-19).
...
app.post("/api/anti-iptv/params", async (req, res) => {
  try {
    ...
    var body = req.body || {};
    var updates: Record<string, number> = {};
    if (body.maxUsername !== undefined && body.maxUsername !== null && body.maxUsername !== "") updates.maxUsername = parseInt(body.maxUsername, 10);
    if (body.windowSeconds !== undefined && body.windowSeconds !== null && body.windowSeconds !== "") updates.windowSeconds = parseInt(body.windowSeconds, 10);
    if (body.banSeconds !== undefined && body.banSeconds !== null && body.banSeconds !== "") updates.banSeconds = parseInt(body.banSeconds, 10);
    ...
    var patterns = ANTI_IPTV_FIELD_PATTERNS[variant];
    var applied: Record<string, number> = {};
    var skipped: string[] = [];
    ...
    for (var ui = 0; ui < fieldNames.length; ui++) {
      ...
      if (!m) { skipped.push(field); continue; }
      var fullMatch = m[0];
      var valueGroup = m[1];
      ...
      content = content.slice(0, (m.index as number)) + patchedMatch + content.slice((m.index as number) + fullMatch.length);
      applied[field] = updates[field];
    }

    if (Object.keys(applied).length === 0) {
      return res.status(422).json({ error: "Nessun campo riconosciuto nello script live", skipped: skipped });
    }
    ...
    res.json({ ok: true, applied: applied, skipped: skipped, restartWarning: restartWarning || undefined });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
});
...
app.get("/api/anti-iptv/status", async (_req, res) => {
  try {
    ...
    res.json({ running: running, ipCount: ipCount });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
});
...
app.get("/api/anti-iptv/banned", async (_req, res) => {
  try {
    ...
    var activeIpArr: string[] = (ipsetR.ok && ipsetR.stdout.trim() ? ipsetR.stdout.split("\n") : [])
      .map(function(l: string) { return l.trim(); })
      .filter(function(l: string) { return /^\d+\.\d+\.\d+\.\d+$/.test(l); });
    var activeIps: { [ip: string]: boolean } = {};
    for (var ai = 0; ai < activeIpArr.length; ai++) { activeIps[activeIpArr[ai]] = true; }

    var logR = await runCmd("cat /var/log/anti-iptv/bans.log 2>/dev/null || echo ''");
    var entries: Array<{ ip: string; date: string; usernames: string[]; active: boolean }> = [];
    var seenIp: { [ip: string]: number } = {};
    ...
    if (logR.ok && logR.stdout.trim()) {
      ...
      for (var bi = 0; bi < blocks.length; bi++) {
        ...
        var usernamesSeen: { [u: string]: boolean } = {};
        ...
        if (seenIp[ip] !== undefined) {
          ...
          entries.push({ ip: ip, date: date, usernames: usernames, active: !!activeIps[ip] });
        }
      }
    ...
    res.json({ entries: entries, activeCount: activeIpArr.length });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
});
...
app.post("/api/crowdsec/install", async (req, res) => {
  ...
  var centralLapi: { url: string; login: string; password: string; bouncerKey: string } | null = null;
  var raw = req.body && req.body.centralLapi;
  if (
    raw &&
    /^https?:\/\/[\w.:-]+\/?$/.test(raw.url || "") &&
    /^[\w-]+$/.test(raw.login || "") &&
    /^[\w-]+$/.test(raw.password || "") &&
    /^[A-Za-z0-9+/=_-]+$/.test(raw.bouncerKey || "")
  ) {
  ...
  try {
    ...
    if (centralLapi) {
      ...
      await writeFile("/tmp/pg-cs-central-creds.yaml", credsYaml, { encoding: "utf-8", mode: 0o600 });
      var credsWrite = await runCmd(
        "cat /tmp/pg-cs-central-creds.yaml | sudo tee /etc/crowdsec/local_api_credentials.yaml > /dev/null",
        5000
      );
      try { await unlink("/tmp/pg-cs-central-creds.yaml"); } catch {}
    ...
    if (centralLapi && confRead.ok) {
      var centralConf = confRead.stdout
        .replace(/api_key:.*/, "api_key: " + centralLapi.bouncerKey)
        .replace(/api_url:.*/, "api_url: " + centralLapi.url);
      await writeFile("/tmp/pg-bouncer-conf.yaml", centralConf, { encoding: "utf-8", mode: 0o600 });
      var centralConfWrite = await runCmd("cat /tmp/pg-bouncer-conf.yaml | sudo tee " + bouncerConf + " > /dev/null", 5000);
      try { await unlink("/tmp/pg-bouncer-conf.yaml"); } catch {}
      ...
      if (bouncerKey.ok && key) {
        ...
        await writeFile("/tmp/pg-bouncer-conf.yaml", newConf, { encoding: "utf-8", mode: 0o600 });
...
const AUTOHEAL_COOLDOWN = 10 * 60 * 1000;
    ...
    try {
      ...
      if (s.status !== "running") {
        ...
        if (now - last > AUTOHEAL_COOLDOWN) {
          ...
          await runCmd("sudo systemctl restart " + svc, 30000);
